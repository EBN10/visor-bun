...
import { isFeatureLayerKind } from "~/lib/layer-config";
...
export function LayersProvider(props: { children: React.ReactNode }) {
  ...
  const [mapViewport, setMapViewport] = useState<MapViewportSnapshot | null>(
    null,
  );
  ...
  const setLayerVisibility = useCallback((id: string, visible: boolean) => {
    setVisibleLayerIds((previous) => {
      ...
      if (
        ...
        [...next].every((itemId) => previous.has(itemId))
      ) {
        return previous;
      }
  ...
  const zoomToLayer = useCallback(
    async (id: string) => {
      ...
      if (meta.kind !== "vector" && meta.kind !== "wfs") {
        return;
      }
