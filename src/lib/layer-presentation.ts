...
function getPopupCandidate(
  ...
  return candidates.find((candidate) => {
    ...
    return formatPopupValue(value).trim().length > 0;
  });
}
...
export function formatPopupValue(value: unknown): string {
  ...
  if (Array.isArray(value)) {
    return value
      .map((entry) => formatPopupValue(entry))
      .filter(Boolean)
      .join(", ");
  }
...
export function normalizeLayerBounds(
  ...
  if (Array.isArray(bounds)) {
    ...
    if (
      [south, west, north, east].some((value) => Number.isNaN(Number(value)))
    ) {
      return null;
    }
...
export function getLayerAccentColor(layerId: string, config?: LayerConfig) {
  ...
  return DEFAULT_LAYER_PALETTE[
    hashString(layerId) % DEFAULT_LAYER_PALETTE.length
  ]!;
}
...
export function getResolvedVectorStyle({
    ...
    geometryKind === "polygon"
      ? zoom >= 11
        ...
        : zoom >= 9
          ...
          : 0.08
      : 0.7;
  const defaultRadius = zoom >= 12 ? 7 : zoom >= 10 ? 6 : zoom >= 8 ? 5 : 4;
  ...
  return {
    ...
    weight,
    opacity,
    fillOpacity: geometryKind === "line" ? 0 : fillOpacity,
    radius,
...
export function buildPopupHtml(
  ...
  const configuredTitleProp = resolvePropertyKey(
    properties,
  ...
  const orderedFields = popupConfig?.order?.length
    ? popupConfig.order
  ...
  const metadataLink = options?.layerId
    ? `
      <div class="map-popup__footer">
        <button
          type="button"
          class="map-popup__metadata-link"
          data-layer-metadata-id="${escapeHtml(options.layerId)}"
        >
          Más información
        </button>
      </div>
  ...
  if (!titleValue && !subtitleValue && rows.length === 0 && !metadataLink) {
    return null;
  }
